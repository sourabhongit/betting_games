            ...
            <table class="table table-bordered" data-auto-responsive="false" id="current-game">
                <thead>
                    <tr class="nk-tb-item nk-tb-head">
                        ...
                        <th class="nk-tb-col tb-col-md"><span class="sub-text">Bet on Tail</span></th>
                        <th class="nk-tb-col tb-col-md"><span class="sub-text">Bet On Head</span></th>
                ...
                <tbody>
                    <tr class="nk-tb-item">
                        ...
                        <td class="nk-tb-col" id="TailBetAmount">{{ $TailBetAmount }}</td>
                        <td class="nk-tb-col" id="HeadBetAmount">{{ $HeadBetAmount }}</td>
                        @php
                            if ($GameLog) {
                        ...
                        <td class="nk-tb-col"><button class="btn btn-primary"
                                onclick="UpdateResult('head', {{ $GameLog->id }})">Head</button>
                            <button class="btn ml-2 btn-secondary"
                                onclick="UpdateResult('tail', {{ $GameLog->id }})">Tail</button>
                        </td>
                    </tr>
                </tbody>
            </table>
            ...
            <table class="datatable-init nowrap nk-tb-list nk-tb-ulist" data-auto-responsive="false">
                <thead>
                    <tr class="nk-tb-item nk-tb-head">
                        ...
                        <th class="nk-tb-col tb-col-md"><span class="sub-text">Game Log ID</span></th>
    ...
    @push('custom-js')
        <script>
            function UpdateResult(Result, LogID) {
                $.ajax({
                    url: '{{ route('head.tail.update.result') }}',
                    type: 'POST',
                    data: {
                        Result: Result,
                        GameLogId: LogID
            ...
            function FetchGameData() {
                $.ajax({
                    url: '{{ route('head.tail.running.game.data', ['id' => $GameLog->id]) }}',
                    type: 'GET',
                    success: function(response) {
                        $('#TotalPlayers').text(response.TotalPlayes);
                        $('#TailBetAmount').text(response.TailBetAmount);
                        $('#HeadBetAmount').text(response.HeadBetAmount);
                    },
                    error: function(xhr, status, error) {
